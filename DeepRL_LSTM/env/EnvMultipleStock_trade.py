...
class StockEnvTrade(gym.Env):
    ...
    def _buy_stock(self, index, action):
        ...
        if self.turbulence< self.turbulence_threshold:
            ...
            print('buy available_amount:{}'.format(available_amount))
            

            tmpx = self.state[0] - self.state[index+1]*min(available_amount, action)* \
                   (1+ TRANSACTION_FEE_PERCENT)
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            print('#'*30)
            print('term_all_stocks {}, Asset {}'.format(sum(np.array(self.state[(STOCK_DIM+1):(STOCK_DIM+2+1)])), end_total_asset))
            #print('all_stocks {}'.format(sum(np.array(self.state[(STOCK_DIM+1):(STOCK_DIM+2+1)]))))
            print('#'*30)
            ...
            sum(np.array(self.state[1:(STOCK_DIM+1)])*np.array(self.state[(STOCK_DIM+1):(STOCK_DIM*2+1)]))
            ac=np.array(actions)
            print('all_stocks {}, Asset {} sell:{} buy:{}'.format(sum(np.array(self.state[(STOCK_DIM+1):(STOCK_DIM+2+1)])), end_total_asset,sum(ac[ac<0]),sum(ac[ac>0])))
    ...
    def reset(self):  
        if self.initial:
            ...
            print(self.previous_state[1:(STOCK_DIM+1)])
            print(self.previous_state[(STOCK_DIM+1):(STOCK_DIM*2+1)])
            print('@'*20)
