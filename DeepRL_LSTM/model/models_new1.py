...
def train_A2C(env_train, model_name, timesteps=25000):
    ...
    start = time.time()
    model = A2C('MlpPolicy', env_train, verbose=1, tensorboard_log='runs',full_tensorboard_log=True)
...
def train_PPO(env_train, model_name, timesteps=50000):
    ...
    start = time.time()
    #model = PPO2('MlpPolicy', env_train, ent_coef = 0.005, verbose=1, nminibatches = 8, tensorboard_log='runs', full_tensorboard_log=True)
    model = PPO2('MlpLnLstmPolicy', env_train, ent_coef = 0.005, nminibatches=1, gamma=0.93, learning_rate=0.005)
...
def DRL_prediction(df,
    ...
    env_trade = make_vec_env(lambda: StockEnvTrade(trade_data,
                                                   turbulence_threshold=turbulence_threshold,
                                                   initial=initial,
                                                   previous_state=last_state,
                                                   model_name=name,
                                                   iteration=iter_num), n_envs=1)
...
def run_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window) -> None:
    ...
    for i in range(rebalance_window + validation_window, len(unique_trade_date), rebalance_window):
        ...
        from stable_baselines.common import make_vec_env
        env_train = make_vec_env(lambda: StockEnvTrain(train), n_envs=1 )
        ...
        env_val = make_vec_env(lambda: StockEnvValidation(validation,
                                                          turbulence_threshold=turbulence_threshold,
                                                          iteration=i), n_envs=1)
        ...
        print("======PPO Training========")
        model_ppo = train_PPO(env_train, model_name="PPO_100k_dow_{}".format(i), timesteps=1000000)
